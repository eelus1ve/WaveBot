...
class FunSetup(commands.Cog, ):
    ...
    def __ctx_to_color(ctx):
        return SQLEditor.get_color(ctx)(name="FUN")

    @commands.command()
    async def dice(self, ctx: commands.Context):
        await Dice(self.bot, self.__ctx_to_color(ctx)).command_dice(ctx)

    @commands.command(aliases=['монетка', 'Монетка', 'МОНЕТКА'])
    async def coin(self, ctx: commands.Context):
        await Coin(self.bot, self.__ctx_to_color(ctx)).command_coin(ctx)

    @commands.command(aliases=['Волшебный_шар', '8ball', 'MagicBall', '8Ball'])
    async def Magicball(self, ctx: commands.Context):
        await MagicBall(self.bot, self.__ctx_to_color(ctx)).command_Magicball(ctx)

    @commands.command()
    async def p2048(self, ctx: commands.Context):
        await Game2048(self.bot, self.__ctx_to_color(ctx)).command_p2048(ctx)
    ...
    async def send_an_message(self, ctx: commands.Context):
        await Get_message(self.bot, self.__ctx_to_color(ctx)).command_send_an_message(ctx)

    @commands.Cog.listener('on_interaction')
    async def on_button_click_2048(self, interaction: discord.Interaction):
        await Game2048(self.bot, self.__ctx_to_color(interaction)).listener_on_button_2048(interaction)

    @commands.Cog.listener('on_button_click')
    async def on_button_click_anMessage(self, interaction: discord.Interaction):
        await Get_message(self.bot, self.__ctx_to_color(interaction)).listener_on_button_click_anMessage(interaction)
