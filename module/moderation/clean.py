...
class Cleanpy(commands.Cog):
    ...
    async def clear(self, ctx, amount: int):
        ...
        if quest:
            ...
            if amount <= 1000:
                ...
                await embpy(ctx, comp='s', des="*Очищено* " + str(amount) + " *сообщений*", time=5.0)
                
            else:
                await embpy(ctx, comp='e', des=f"*Использование:* {pref}*clear (количество до 1000 за 1 раз)*", time=10.00)
    ...
    async def error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await embpy(ctx, comp='e', des="*У вас недостаточно прав!*", time=10.00)
                

    @commands.command()#удалить надо будет
