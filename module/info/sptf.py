...
class Sptfpy(commands.Cog):
    ...
    async def spotify(self, ctx, user: discord.Member = None):
        ...
        titleFont = ImageFont.truetype('allFonts.otf', 16)
        artistFont = ImageFont.truetype('allFonts.otf', 14)
        startFont = ImageFont.truetype('allFonts.otf', 12)
        albFont = artistFont
        endFont = startFont
        ...
        end_position = 515, 122
        draw.text(end_position, f"{dateutil.parser.parse(str(spotifyActivity.duration)).strftime('%M:%S')}", clr, font=endFont)
        ...
        await ctx.send(file=File('spotify.jpg'))
