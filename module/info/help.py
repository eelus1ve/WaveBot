...
class Helppy(commands.Cog):
        ...
        if arg:
            await ctx.author.send(embed=discord.Embed(
                ...
                description=f"*Использование: {pref} {arg}\n\
                    {[i[arg] for i in Helppy.help_dict if arg in i]}*",
                color=COLOR
            ))
            ...
            var = list(Helppy.help_dict[list(Helppy.help_dict.keys())[0]].keys())
            emb = discord.Embed(title='*Список доступных команд:*',
                description = '',
                color=COLOR)
            emb.add_field(name='', value='**`модерация`**' + '\n' + f'\n'.join([i for i in Helppy.help_dict if i != 'модерация']))
            emb.add_field(name='', value=f'**`{var[0]}`**' + '\n' + f'\n'.join([i for i in var if i != var[0]]))
            emb.add_field(name='='*len(Helppy.help_dict['модерация']['warn']), value=f"{Helppy.help_dict['модерация']['warn']}", inline=True)
    ...
    async def help_scroll(self, interaction: discord_components.Interaction):
        COLOR = 0x0000FF
        pref = '~'
        ...
        if interaction.component.label == '→':
            ...
            emb.add_field(name='='*len(Helppy.help_dict[first_key][des_name]), value=f'{Helppy.help_dict[first_key][des_name]}', inline=True)
        ...
        if interaction.component.label == '←':
            ...
            emb.add_field(name='', value=old_field0_value)
            emb.add_field(name='', value=f'\n'.join(old_field1_value))
            emb.add_field(name='=' * len(Helppy.help_dict[first_key][des_name]),
                          value=f'{Helppy.help_dict[first_key][des_name]}', inline=True)
        ...
        if interaction.component.label == '↑':
            ...
            var = list(Helppy.help_dict[first_key].keys())
            ...
            emb.add_field(name='', value=f'**`{var[0]}`**' + '\n' + f'\n'.join([i for i in var if i != var[0]]))
            emb.add_field(name='='*len(Helppy.help_dict[first_key][var[0]]), value=f'{Helppy.help_dict[first_key][var[0]]}', inline=True)
        ...
        if interaction.component.label == '↓':
            ...
            emb.add_field(name='', value='\n'.join(old_field0_value))
            emb.add_field(name='', value=f'**`{var[0]}`**' + '\n' + f'\n'.join([i for i in var if i != var[0]]))
            emb.add_field(name='=' * len(Helppy.help_dict[first_key][var[0]]),
                          value=f'{Helppy.help_dict[first_key][var[0]]}', inline=True)
