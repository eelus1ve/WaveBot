...
class SQL_write(commands.Cog):
    def __init__(self, bot):
        ...
        # cursor.execute(f"DROP DATABASE {self.namedb}")


        
        cursor.execute(f"CREATE DATABASE IF NOT EXISTS {self.namedb}")
        cursor.close()
        # quit()
        self.db = mysql.connector.connect(
            host=self.host,
    ...
    def write_db(self):
        ...
        self.conn.execute("""INSERT IGNORE INTO server(server_id) VALUES ?""", full_inserts['server'])
        self.db.commit()

        self.conn.executemany("""INSERT IGNORE INTO color(name) VALUES ?""", full_inserts['color'])
        self.db.commit()

        self.conn.executemany("""INSERT IGNORE INTO module(name) VALUES ?""", full_inserts['module'])
        self.db.commit()

        self.conn.executemany("""INSERT IGNORE INTO list_type(type) VALUES ?""", full_inserts['list_type'])
        self.db.commit()

        self.conn.executemany("""INSERT IGNORE INTO server_prefix(server_id, prefix) VALUES (SELECT id FROM server WHERE server_id = %s LIMIT 1), %s""", full_inserts['server_prefix'])
        self.db.commit()
        ...
        self.conn.executemany('''INSERT INTO user(discord_id, tg_link, anilagann) VALUES ?''', full_inserts['user'])

        self.db.commit()

        self.conn.executemany('''INSERT INTO server_user(server_id, user_id, IS_OWNER, warns, caps, xp, time) 
    ...
    def newmembersql(self, member: discord.Member):
        self.conn.execute('''INSERT INTO user(discord_id, tg_link, anilagann) 
    ...
    def memberjoinsql(self, member: discord.Member):
        self.conn.execute('''INSERT INTO server_user(server_id, user_id, IS_OWNER, warns, caps, xp, time) 
    ...
    def newguildsql(self, guild):
        ...
        # print(guild.id)
        self.conn.execute("""INSERT INTO server(server_id)
